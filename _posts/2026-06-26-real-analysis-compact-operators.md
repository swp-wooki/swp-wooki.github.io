...
$$B = \{f \in H : \vert \vert f\vert \vert  \le r\} \subset H$$
is compact if and only if $$\dim H < \infty$$.
</li>
<li markdown="1">In an infinite dimensional space, the orthonormal set $$\{e_n\}$$ has no converging subsequence because $$\vert \vert e_n - e_m\vert \vert = \sqrt{2}$$ for all $$n \neq m$$.
...
$$
\begin{aligned}
\vert \vert T(g_k) - T(g_l)\vert \vert  &\le \vert \vert T(g_k) - T_m(g_k)\vert \vert  + \vert \vert T_m(g_k) - T_m(g_l)\vert \vert  + \vert \vert T_m(g_l) - T(g_l)\vert \vert  \\
&= I_1 + I_2 + I_3
\end{aligned}
$$
...
We claim that
$$\vert \vert Q_n T\vert \vert  \to 0 \quad \text{as } n \to \infty.$$
If not, then there exists $$c > 0$$ such that
$$\vert \vert Q_n T\vert \vert  \ge c \quad \text{ for all } n \in \mathbb{N},$$
since $$\vert \vert Q_n g\vert \vert \searrow 0 $$ as $$n \to \infty$$ for all $$g \in H$$.

Hence, for each $$n$$,
there exists $$f_n$$ with $$\vert \vert f_n\vert \vert = 1$$ such that $$\vert \vert Q_n T f_n\vert \vert \ge c$$.
...
$$
\vert \vert Q_{n_k}(g)\vert \vert  > \frac{c}{2} \quad \text{for large } k.
$$
...
$$\vert \vert T^* P_n - T^*\vert \vert  = \vert \vert P_n T - T\vert \vert  \to 0.$$
...
Note that $$\vert \vert T\vert \vert \le \vert \vert K\vert \vert _{L^2(\mathbb{R}^d \times \mathbb{R}^d)}$$, since

$$
\vert T(f)(x)\vert  \le \int_{\mathbb{R}^d} \vert K(x,y)\vert  \vert f(y)\vert  dy \le \left(\int_{\mathbb{R}^d} \vert K(x,y)\vert ^2 dy\right)^{1/2} \vert \vert f\vert \vert _{L^2},
$$

we obtain

$$
\vert \vert Tf\vert \vert ^2 \le \int \left(\int \vert K(x,y)\vert ^2 dy\right) \vert \vert f\vert \vert ^2 dx = \vert \vert K\vert \vert _{L^2}^2 \vert \vert f\vert \vert ^2.
$$
...
$$
K(x,y) \sim \sum_{k,l} a_{kl} \varphi_k(x) \varphi_l(y) \quad \text{with } \sum_{k,l} \vert a_{kl}\vert ^2 < \infty.
$$
...
$$
\vert \vert K - K_n\vert \vert _{L^2(\mathbb{R}^d \times \mathbb{R}^d)}^2 = \sum_{\overset{k \ge n}{\text{or} \; l \ge n}} \vert a_{k,l}\vert ^2 \to 0,
$$

we have
$$\vert \vert T - T_n\vert \vert  \le \vert \vert K - K_n\vert \vert  \to 0.$$
...
$$\vert \vert \varphi_{n_k} - \varphi_{n_l}\vert \vert  = \sqrt{2} \quad \text{for all } k \neq l.$$
</li>
<li markdown="1">Suppose to the contrary that for each $$\mu > 0$$, there exists a sequence $$\{\lambda_n\}$$ of distinct eigenvalues such that $$\vert \lambda_n\vert > \mu$$.
...
$$
\vert \vert \lambda_{n_k}\varphi_{n_k} - \lambda_{n_l}\varphi_{n_l}\vert \vert ^2 = \lambda_{n_k}^2 + \lambda_{n_l}^2 > 2\mu^2 > 0,
$$
...
Suppose $$T \neq 0$$ is compact and symmetric. Then either $$\vert \vert T\vert \vert $$ or $$-\vert \vert T\vert \vert $$ is an eigenvalue of $$T$$.
...
$$
\vert \vert T\vert \vert  = \sup \{(Tf, f) : \vert \vert f\vert \vert  = 1\} \text{ or } -\vert \vert T\vert \vert  = \inf \{(Tf, f) : \vert \vert f\vert \vert  = 1\}
$$

Assume the first case, and $$\lambda = \vert \vert T\vert \vert $$. Then there exists $$\{f_n\} \subset H$$ such that
$$\vert \vert f_n\vert \vert = 1$$ and $$(Tf_n, f_n) \to \lambda$$.
...
$$
\begin{aligned}
\vert \vert Tf_{n_k} - \lambda f_{n_k}\vert \vert ^2 &= \vert \vert Tf_{n_k}\vert \vert ^2 - 2\lambda(Tf_{n_k}, f_{n_k}) + \lambda^2 \vert \vert f_{n_k}\vert \vert ^2 \\
...
If $$g = 0$$, we get
$$\lambda = \lim_{k \to \infty} (Tf_{n_k}, f_{n_k}) = 0$$ which implies that $$\vert \vert T\vert \vert = 0 \implies T = 0$$, a contradiction to $$T \neq 0$$.
