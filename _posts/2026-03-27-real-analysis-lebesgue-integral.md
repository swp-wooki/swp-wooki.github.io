...
<li markdown="1">(Triangle inequality) $$\displaystyle \left\vert \int \varphi\right\vert \le \int \vert \varphi\vert $$.
...
ETS : $$\left\vert \int \varphi_n - \int \varphi_m\right\vert \le \int \vert \varphi_n - \varphi_m\vert < \epsilon$$
...
$$
\vert \varphi_n(x) - \varphi_m(x)\vert  < \epsilon \quad \forall \: n, m \ge n_0 \;, \: \forall \: x \in A_\epsilon.
$$
...
$$
\begin{aligned}
\left\vert \int \varphi_n - \int \varphi_m\right\vert  &\le \int_E \vert \varphi_n - \varphi_m\vert  \\
&= \int_{A_\epsilon} \vert \varphi_n - \varphi_m\vert  + \int_{E \setminus A_\epsilon} \vert \varphi_n - \varphi_m\vert  \\
...
ETS : $$\left\vert \int \varphi_n - 0\right\vert = \left\vert \int \varphi_n\right\vert \le \int \vert \varphi_n\vert < \epsilon$$
The same proof shows that

$$
\begin{aligned}
\left\vert \int \varphi_n\right\vert  &\le \int_{A_\epsilon} \vert \varphi_n\vert  + \int_{E \setminus A_\epsilon} \vert \varphi_n\vert  \\
...
<li markdown="1">$$\vert \varphi_n\vert \le M$$ for some $$M > 0$$,
</li>
...
$$
\vert \eta_n\vert  \le 2M, \quad m(\supp \eta_n) < \infty \quad \text{and } \eta_n \to 0 \: \text{ a.e. as } n \to \infty.
$$
...
Let $$\{f_n\}$$ be a sequence of measurable functions s.t. $$\vert f_n\vert \le M$$, $$\text{supp}(f_n) \subset E \: \: \forall n \in \mathbb{N}$$ with $$m(E) < \infty$$, and $$f_n \to f$$ a.e. on $$E$$ as $$n \to \infty$$.

Then $$f$$ is measurable, bounded a.e., supported on $$E$$ ($$\text{supp}(f) \subset E$$) a.e., and

$$
\int \vert f_n - f\vert  \to 0 \quad \text{as } n \to \infty
$$
...
$$\left\vert \int \vert f_n - f\vert - 0\right\vert = \left\vert \int \vert f_n - f\vert \right\vert = \int \vert f_n - f\vert < \epsilon$$
...
Furthermore, since $$\vert f_n\vert \le M$$ for all $$n$$, we have

$$
\vert f_n(x) - f(x)\vert  \le \vert f_n(x)\vert  + \vert f(x)\vert  \le 2M \quad \text{ for all } n \in \mathbb{N}, \: x \in E.
$$
...
$$
\begin{aligned}
\int \vert f_n - f\vert  &= \int_{A_\epsilon} \vert f_n - f\vert  + \int_{E \setminus A_\epsilon} \vert f_n - f\vert  \\
&\le \int_{A_\epsilon} \epsilon + \int_{E \setminus A_\epsilon} 2M \\
...
for all $$n \ge n_0$$. Since $$\epsilon$$ was arbitrary, $$\int \vert f_n - f\vert \to 0$$ as $$n \to \infty$$. Moreover,

$$
\left\vert \int f_n - \int f\right\vert  \le \int \vert f_n - f\vert  \to 0 \quad \text{ as } n \to \infty.
$$
...
*Proof.*
The idea of proof is to make the measure of the set $$\{x : f(x) > 0\}$$ be zero. <br>
...
$$
\lim_{k \to \infty} \int_a^b \varphi_k = \int_a^b f = \lim_{k \to \infty} \int_a^b \psi_k \qquad (\ast)
$$
...
$$
\int_a^b \varphi_k = \int_{[a,b]} \varphi_k \quad \text{and} \quad \int_a^b \psi_k = \int_{[a,b]} \psi_k \quad \forall \: k \qquad (\ast\ast)
$$
...
$$
\begin{aligned}
\int_{[a,b]} \tilde{\varphi} &= \lim_{k \to \infty} \int_{[a,b]} \varphi_k \qquad (\because BCT)\\
&= \lim_{k \to \infty} \int_{[a,b]} \psi_k \qquad (\because (\ast) and (\ast\ast)) \\
&= \int_{[a,b]} \tilde{\psi} \qquad (\because BCT)
\end{aligned}
$$
...
&= \lim_{k \to \infty} \int_a^b \varphi_k \qquad (\because (\ast\ast))\\
&= \int_a^b f.\qquad (\because (\ast))
\end{aligned}
$$
...
<li markdown="1">WTS : $$\int (af + bg) = a\int f + b\int g \quad$$ if $$f, g \ge 0 $$ and $$ a, b \ge 0$$. <br>
...
If $$\{f_n\}$$ is a sequence of measurable functions with $$f_n \ge 0$$. <br>
...
$$
\int \sum_{k=1}^\infty a_k(x) dx = \sum_{k=1}^\infty \int a_k(x) dx \qquad \text{(*)}
$$
...
**Definition.**
$$f$$ is (Lebesgue) integrable if $$\vert f\vert $$ is integrable. Its (Lebesgue) integral is defined by
...
$$\int_{B^c} \vert f\vert  < \epsilon$$
</li>
<li markdown="1">$$\exists \: \delta > 0$$ such that
$$\int_E \vert f\vert  < \epsilon \quad \text{whenever } m(E) < \delta.$$
...
&\le \int (f - f_{N_0}) + \int_E N_0 \qquad (\because f_{N_0} \le N_0) \\
...
$$
f(x) \to 0 \quad \text{as } \vert x\vert  \to \infty.
$$
...
Suppose that $$f_n \to f$$ a.e.  <br>
If $$\vert f_n\vert \le g$$ for some $$g$$ integrable, then

$$
\lim_{n \to \infty} \int \vert f_n - f\vert  = 0, \quad \text{and}
$$
...
$$
\int \vert f_n - f\vert  \to 0 \implies \int_{E_{N_0}} \vert f_n - f\vert  < \frac{\epsilon}{3} \qquad \text{for all large } n.
$$
...
$$
\begin{aligned}
\int \vert f_n - f\vert  &= \int_{E_{N_0}} \vert f_n - f\vert  + \int_{E_{N_0}^c} \vert f_n - f\vert  \\
&\le \int_{E_{N_0}} \vert f_n - f\vert  + \int_{E_{N_0}^c} \vert f_n\vert  + \int_{E_{N_0}^c} \vert f\vert  \\
&\le \int_{E_{N_0}} \vert f_n - f\vert  + \int_{E_{N_0}^c} g + \int_{E_{N_0}^c} g \\
...
$$
\vert f\vert  = (u^2 + v^2)^{1/2} \quad \text{is integrable.}
$$
...
since $$\vert u\vert , \vert v\vert \le \vert f\vert $$ and $$\vert f\vert \le \vert u\vert + \vert v\vert $$.
