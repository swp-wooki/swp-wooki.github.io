...
<li markdown="1">(Parseval's identity) 
    \[
    \sum_{n \in \mathbb{Z}} |a_n|^2 = \frac{1}{2\pi} \int_{-\pi}^\pi |f|^2
    \]
</li>
...
<li markdown="1">The Fourier series of $f$ converges to $f$ in $L^2$, that is,
    \[
    \frac{1}{2\pi} \int_{-\pi}^\pi |f(x) - S_N(f)|^2 dx \to 0 \quad \text{as } N \to \infty
    \]
    where $S_N(f) = \sum_{|n| \le N} a_n e^{inx}$.
...
<li markdown="1">$\sum_{n \in \mathbb{Z}} a_n r^{|n|} e^{inx} \to f(x)$ for a.e. $x$ as $r \nearrow 1$.
...
Since 
\[
\sum |a_n r^{|n|} e^{inx}| \le \left(\frac{1}{2\pi} \int_{-\pi}^\pi |f(x)| dx\right) \sum r^{|n|}
\]
the series $\sum a_n r^{|n|} e^{inx}$ converges absolutely and uniformly (in $x$) for each $r \in [0, 1)$.
...
\[
\sum_{n \in \mathbb{Z}} r^{|n|} e^{iny} = \frac{1 - r^2}{1 - 2r \cos y + r^2} \eqcolon Pr(y) \tag{Poisson kernel}
\]
We extend $f$ as the $2\pi$-periodic function on $\mathbb{R}$. We claim that 
\[
\sum_{n \in \mathbb{Z}} a_n r^{|n|} e^{inx} = \frac{1}{2\pi} \int_{-\pi}^\pi f(x-y) P_r(y) dy.
\]
Indeed, by the DCT,
\begin{align*}
RHS &= \sum r^{|n|} \frac{1}{2\pi} \int_{-\pi}^\pi f(x-y) e^{iny} dy \
&= \sum r^{|n|} \frac{1}{2\pi} \int_{-\pi+x}^{\pi+x} f(y) e^{in(x-y)} dy \
&= \sum r^{|n|} e^{inx} \frac{1}{2\pi} \int_{-\pi}^\pi f(y) e^{-iny} dy = LHS
\end{align*}
...
\[
\sum_{n=0}^\infty |a_n|^2 r^{2n} = \frac{1}{2\pi} \int_{-\pi}^\pi |F(re^{i\theta})|^2 d\theta \quad \text{ for each } r \in (0, 1).
\]
By the boundedness of $F$, letting $r \nearrow 1$ shows that $\sum_{n=0}^\infty |a_n|^2$ converges. 

Now, we define $F(e^{i\theta})$ by the $L^2$ function where Fourier coefficients are $a_n$ when $n \ge 0$, and $0$ when $n < 0$.<span class="real-analysis-footnote">Note: Existence and uniqueness of such $L^2$-function, under the assumption $\{a_n\} \in l^2$, by the Riesz-Fischer theorem.</span>
...
\[
\sup_{0 \le r < 1} \frac{1}{2\pi} \int_{-\pi}^\pi |F(re^{i\theta})|^2 d\theta < \infty
\]
which is weaker than the boundedness. Theorem 3.3 holds for any function in the Hardy space 
\[
H^2(\mathbb{D}) = \left\{ F : \mathbb{D} \to \mathbb{C} \text{ holomorphic} : \sup_{0 \le r < 1} \frac{1}{2\pi} \int_{-\pi}^\pi |F(re^{i\theta})|^2 d\theta < \infty \right\}.
\]
