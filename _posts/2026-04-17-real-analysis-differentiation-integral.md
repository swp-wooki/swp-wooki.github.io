...
From the definition of differentiation, we can observe:
    \begin{align*}
        ...
        &= \lim_{\overset{|I| \to 0}{x \in I}} \frac{1}{|I|} \int_I f(y) dy \
        &\overset{?}{=} f
    \end{align*}
where $I = (x, x+h)$ for some $h > 0$ and $|I|$ denotes the length of $I$.
Therefore, the main question 1 is equivalent to whether the last equality
\[
\lim_{|I| \to 0} \frac{1}{|I|} \int_I f(y) dy = f
\]
holds (at least a.e.) for $f$. Note that this expression
\[
\frac{1}{|I|} \int_I f(y) dy
\]
is the average value of $f$ over $I$, and we expect that its limit tend to $f(x)$ as $|I| \to 0$.
...
\[
\exists \: \delta > 0 \text{ s.t. } |f(x) - f(y)| < \epsilon \text{ whenever } |x - y| < \delta.
\]
Note that
\begin{align*}
...
Indeed,
\begin{align*}
\left| f(x) - \mathop{\rlap{-}\int}_B f(y) dy \right|
&= \left| \mathop{\rlap{-}\int}_B f(x)dy - \mathop{\rlap{-}\int}_B f(y)dy \right| \
&\le \mathop{\rlap{-}\int}_B |f(x) - f(y)| dy < \epsilon
\end{align*}
...
\[
f^*(x) = \sup_{x \in B} \mathop{\rlap{-}\int}_B |f(y)| dy
\]
...
<li markdown="1">$f^*$ satisfies
    \[
    m(\{x \in \mathbb{R}^d : f^*(x) > \alpha\}) \le \frac{3^d}{\alpha} ||f||_{L^1(\mathbb{R}^d)} \quad \forall \; \alpha > 0
    \] 
...
<li markdown="1">(**Tchebychev inequality**) In the previous chapter, we have proved that
        \[
        \text{for all } g \in L^1(\mathbb{R}^d) \text{ and } \alpha > 0, \quad m(\{ |g| > \alpha \}) \le \frac{1}{\alpha} ||g||_{L^1(\mathbb{R}^d)} \quad \text{(weak-type)}
        \]
</li>
<li markdown="1">We will see in Theorem 1.3 that
        \[
        f^{*}(x) \ge |f(x)| \quad \text{for a.e. } x.
        \]
</li>
...
<li markdown="1">Observe that $E_\alpha = \{x : f^*(x) > \alpha\}$ is open. Indeed, if $\overline{x} \in E_\alpha$, then
    \[
    f^*(\overline{x}) = \sup_{\overline{x} \in B} \mathop{\rlap{-}\int}_B |f(y)| dy > \alpha
    \]
    holds, we can pick such a ball $B \subset E_\alpha$ so that
    \[
    \overline{x} \in B \quad \text{ and } \quad \mathop{\rlap{-}\int}_B |f(y)| dy > \alpha
    \]
...
<li markdown="1">This follows from (iii) : Indeed, since
    ...
    we have
    \[
    m(\{f^* = \infty\}) \le m(\{f^* > \alpha\}) \le \frac{3^d}{\alpha} ||f||_{L^1}.
    \]
    Letting $\alpha \to \infty$ shows that $m(\{f^* = \infty\}) = 0$.
</li>
<li markdown="1">For each $x \in E_\alpha = \{f^* > \alpha\}$, there exists a ball $B_x \ni x$ such that
    \[
    \mathop{\rlap{-}\int}_{B_x} |f| > \alpha, \quad \text{i.e. } m(B_x) < \frac{1}{\alpha} \int_{B_x} |f|.
    \]
    ...
    &\le \frac{3^d}{\alpha} \sum_{j=1}^k \int_{B_{i_j}} |f| = \frac{3^d}{\alpha} \int_{\bigcup_{j=1}^k B_{i_j}} |f| \le \frac{3^d}{\alpha} \int_{\mathbb{R}^d} |f|
    \end{align*}
...
*Proof.*
ETS : $m(E_\alpha) = 0$ for all $\alpha > 0$<span class="real-analysis-footnote">Note: Since if $m(E_\alpha) = 0 \; \forall \alpha > 0 \implies m(E) = 0$ where $E = \bigcup_{n=1}^\infty E_{\frac{1}{n}}$. Then,
\[
...
\[
E_{\frac{1}{n}}^c = \left\{x : \limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B f(y) dy - f(x) \right| \le \frac{2}{n} \right\}
\]
therefore, if $x \notin E$, then desired limit holds. Since $m(E) = 0$, we conclude that the limit holds for a.e. $x$.</span>, where
\[
E_\alpha := \left\{x : \underbrace{\limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B f(y) dy - f(x) \right|}_{(\ast)} > 2\alpha \right\}
\]
Fix $\alpha > 0$ and let $\epsilon > 0$. Then
\[
\exists \; g \in C_c^0(\mathbb{R}^d) \text{ such that } ||f - g||_{L^1} < \epsilon  \tag{$\ast$}
\]
<span class="real-analysis-footnote">Note: Theorem 2.4 in chapter 2 states that $C_c^0(\mathbb{R}^d)$ is dense in $L^1(\mathbb{R}^d)$. Recall that the definition of density on $L^1$ space is as follows:
$\mathcal{F}$ is dense in $L^1$ if for all $f \in L^1$ and $\epsilon > 0$, there exists $g \in \mathcal{F}$ such that $||f - g||_{L^1} < \epsilon$.</span>
...
\begin{align*}
(\ast) = &\limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B (f(y) - g(y)) dy + \mathop{\rlap{-}\int}_B g(y) dy - g(x) + g(x) - f(x) \right| \
&\le \limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B (f(y) - g(y)) dy \right| + \limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B g(y) dy - g(x) \right| + |g(x) - f(x)| \
&\le (f - g)^*(x) + |f(x) - g(x)|
\end{align*}
since
\[
\limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B (f(y) - g(y)) dy \right| \le (f - g)^*(x) \quad \text{and} \quad \limsup_{\overset{m(B) \to 0}{x \in B}} \left| \mathop{\rlap{-}\int}_B g(y) dy - g(x) \right| = 0,
\]
...
\[
2\alpha < (\ast) \le (f - g)^*(x) + |f(x) - g(x)| \implies 2\alpha < (f - g)^*(x) + |f(x) - g(x)|
\]
since both are positive, at least one of them is greater than $\alpha$. <span class="real-analysis-footnote">Note: If $a, b > 0$ and $a + b > 2\alpha$, then either $a > \alpha$ or $b > \alpha$. If not, then $a \le \alpha$ and $b \le \alpha$, thus $a + b \le 2\alpha$, which is a contradiction.</span>

Let 
\[
F_\alpha = \{x : (f - g)^*(x) > \alpha\} \quad \text{and} \quad G_\alpha = \{x : |f(x) - g(x)| > \alpha\}.
\]
...
\[
m(G_\alpha) \le \frac{1}{\alpha} ||f - g||_{L^1} < \frac{1}{\alpha} \epsilon
\]
and by the weak type estimate for the maximal function, we have
\[
m(F_\alpha) \le \frac{A}{\alpha} ||f - g||_{L^1} < \frac{A}{\alpha} \epsilon
\]
Thus,
\[
...
<li markdown="1">Theorem 1.3 (which applied to $|f|$) implies that
        \[
        f^*(x) \ge |f(x)| \quad \text{for a.e. } x.
        \]
        indeed, 
        ...
        then
        \[
        f^*(x) = \sup_{x \in B} \mathop{\rlap{-}\int}_B |f(y)| dy \ge \lim_{\overset{m(B) \to 0}{x \in B}} \mathop{\rlap{-}\int}_B |f(y)| dy = |f(x)| \quad \text{for a.e. } x.
        \]
...
<li markdown="1">following holds:
    \[
    \lim_{\overset{m(B) \to 0}{\overline{x} \in B}} \mathop{\rlap{-}\int}_B |f(y) - f(\overline{x})| dy = 0
    \]
...
*Proof.*
We apply Theorem 1.4 to $|f(x) - r| \in L^1_{loc}(\mathbb{R}^d)$: Since $|f(x) - r| \in L^1_{loc}$ for each $r \in \mathbb{Q}$, we have
\[
\lim_{\overset{m(B) \to 0}{x \in B}} \mathop{\rlap{-}\int}_B |f(y) - r| dy = |f(x) - r| \quad \text{for a.e. } x.
\]
Now let denote $E_r$ which contains all points $x$ such that the above limit does not hold. That is,
\[
m(E_r) = 0 \quad \text{ and } \quad \lim_{\overset{m(B) \to 0}{x \in B}} \mathop{\rlap{-}\int}_B |f(y) - r| dy = |f(x) - r| \quad \forall x \notin E_r.
\]
Let 
\[
...
\[
\text{WTS : If } x \notin E, \text{ then } x \text{ is a Lebesgue point. \textit{i.e. }} \lim_{\overset{m(B) \to 0}{x \in B}} \mathop{\rlap{-}\int}_B |f(y) - f(x)| dy = 0.
\]
Suppose that $\overline{x} \notin E$ and $f(\overline{x})$ is finite.<span class="real-analysis-footnote">Note: This additional assumption can be justified that if $f$ is locally integrable, then $f$ is finite a.e., which have proved in Proposition 1.6 in Chapter 2.</span> Given $\epsilon > 0$, since $\mathbb{Q}$ is dense in $\mathbb{R}$,
\[
\exists \; r \in \mathbb{Q} \; \; \text{ such that } \; |f(\overline{x}) - r| < \epsilon
\]
Thus,
\begin{align*}
\limsup \mathop{\rlap{-}\int}_B |f(y) - f(\overline{x})| dy &\le \limsup \mathop{\rlap{-}\int}_B |f(y) - r| dy + |f(\overline{x}) - r| \
&= 2|f(\overline{x}) - r| < 2\epsilon.
\end{align*}
Since $0 \le \liminf \mathop{\rlap{-}\int}_B |f(y) - f(\overline{x})| dy$, and $\epsilon$ was arbitrary, we conclude that
\[
\lim_{\overset{m(B) \to 0}{\overline{x} \in B}} \mathop{\rlap{-}\int}_B |f(y) - f(\overline{x})| dy = 0.
\]
...
\[
0 \le \frac{1}{m(U_\alpha)} \le \frac{1}{c m(B)} \quad \text{and} \quad 0 \le \int_{U_\alpha} |f(y) - f(\overline{x})| dy \le \int_B |f(y) - f(\overline{x})| dy.
\]
Since both inequalities are positive, we can multiply them to conclude that
\[
\frac{1}{m(U_\alpha)} \int_{U_\alpha} |f(y) - f(\overline{x})| dy \le \frac{1}{c m(B)} \int_B |f(y) - f(\overline{x})| dy. \tag{$\ast$}
\]
...
\[
\lim_{\substack{m(U_\alpha) \to 0 \ \overline{x} \in U_\alpha}} \mathop{\rlap{-}\int}_{U_\alpha} |f(y) - f(\overline{x})| dy = 0.
\]
