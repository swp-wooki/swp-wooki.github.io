...
\[
L(\gamma) \coloneq \sup_P \sum_{j=1}^N |z(t_j) - z(t_{j-1})| = \inf M
\]
...
<span class="real-analysis-footnote">Note: $F$ is not necessarily continuous.</span>is of **bounded variation** if there exists $M < \infty$ such that for all partition of $[a,b]$,
\[
\sum_{j=1}^N |F(t_j) - F(t_{j-1})| \le M.
\]
We call $\sum_{j=1}^N |F(t_j) - F(t_{j-1})|$ the variation of $F$ with respect to $P$. Furthermore, we define the set of all functions of bounded variation by
\[
...
\]
The results follows from $|a + ib| \le |a| + |b| \le 2|a + ib|$.
...
<li markdown="1">If $F : [a,b] \to \mathbb{R}$ is increasing and bounded, then $F \in BV$ since
    \[
    \sum_{j=1}^N |F(t_j) - F(t_{j-1})| = F(b) - F(a) < 2 \sup |F| < \infty.
    \]
</li>
<li markdown="1">If $F$ is differentiable and $F'$ is bounded, then $F \in BV$. Indeed, by the MVT,
    \[
    \sum |F(t_j) - F(t_{j-1})| = \sum |F'(t_j^*)| (t_j - t_{j-1}) \le M(b-a).
    \]
...
<li markdown="1">The **total variation** of $F$ on $[a,x]$ is defined by
    \[
    T_F(a, x) \coloneq \sup_P \sum_{j=1}^N |F(t_j) - F(t_{j-1})|
    \]
...
we have
\[
|F(x) - F(a) - (P_F - N_F)| < 2\epsilon \implies F(x) - F(a) = P_F(a, x) - N_F(a, x).
\]
...
\begin{align*}
    ...
    &= \sum_{j=1}^N |\cdot| + 2\epsilon \le T_F + 2\epsilon \implies P_F + N_F \le T_F
\end{align*}
...
<li markdown="1">$D^+ F \le D_- F$ a.e.
        ...
        \]
        hence it suffices to show that<span class="real-analysis-footnote">Note: See Exercise 14 to show that $E_\gamma \in \mathcal{M}$.</span>
        \[
...
\[
|F_{n+1} - F_n| < 2^{-n-1} \quad \text{on } [0,1]
\]
...
<li markdown="1">If $f \in L^1$, then
    ...
    <span class="real-analysis-footnote">Note: If $\int_a^b F' = F(b) - F(a)$, then $\int_a^x F' = F(x) - F(a)$ for all $x \in [a,b]$, which implies $F(x) = \int_a^x F' + F(a)$ is absolutely continuous.</span>
...
<span class="real-analysis-footnote">Note: If $\delta \ge m(E)$, then $m(E) - \delta \le 0$</span> Take a compact subset $E'$ of $E$ with $m(E') \ge \delta$, and cover $E'$ by finitely many balls from $\mathcal{B}$.
...
\[
|F(b_x) - F(a_x)| \le \epsilon(b_x - a_x).
\]
...
Therefore,
\begin{align*}
|F(b) - F(a)| &\le \sum_{i=1}^N |F(b_i) - F(a_i)| + \sum_{k=1}^M |F(b_k) - F(a_k)| \
