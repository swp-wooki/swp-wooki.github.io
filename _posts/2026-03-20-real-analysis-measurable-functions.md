...
<li markdown="1">$f$ is measurable $\iff \{f \le a\} \in \mathcal{M} \: \: \forall \: a \in \mathbb{R}$.
    <span class="real-analysis-footnote">Note: Since $(\Rightarrow)$ $\{f \le a\} = \bigcap_k \{f < a + \frac{1}{k}\}$ and $(\Leftarrow)$ $\{f < a\} = \bigcup_k \{f \le a - \frac{1}{k}\}$.</span>
...
<li markdown="1">$\begin{aligned}[t]
    ...
    \end{aligned}$
    <span class="real-analysis-footnote">Note: Note that $\{a < f < b\} = \{x \in E : a < f(x) < b\} = f^{-1}((a, b))$.</span>
...
\end{aligned}$
<span class="real-analysis-footnote">Note: for $f: E \rightarrow \overline{\mathbb{R}}$, $f^{-1}(\pm\infty) \in \mathcal{M}$ is needed</span>
...
\]<span class="real-analysis-footnote">Note: Note that $\sup_n f_n(x)$ is defined by for each fixed $x \in E$, $\sup_n f_n(x) = \sup\{f_n(x) : n \in \mathbb{N}\}$,</span>
are measurable.
...
<li markdown="1">Note that $\{\sup_n f_n > a\} = \bigcup_{n=1}^{\infty} \{f_n > a\}$
    <span class="real-analysis-footnote">Note: $\bigcup_{n=1}^{\infty} \{f_n > a\} = \bigcup_{n=1}^{\infty} \{x \in E : f_n(x) > a\}$)</span>, and we know that each $f_n$  is measurable,
    \[
...
\end{align*} <span class="real-analysis-footnote">Note: Note that $E_{j,k} \text{ and } E_k \in \mathcal{M}$</span>
Then
\[
...
\[
|\varphi_k| \le |f| \quad \text{and} \quad \lim_{k \to \infty} \varphi_k = f.
\]
In particular, $|\varphi_k| \le |f| \: \: \forall \: k$.
...
\[
|\varphi_{k}| = |\varphi_k^{(1)} - \varphi_k^{(2)}| = \varphi_k^{(1)} + \varphi_k^{(2)} \le f^+ + f^- = |f^+ - f^-| = |f| \quad \text{for all } k \in \mathbb{N}
\]
...
<span class="real-analysis-footnote">Note: We make $R_j$ be smaller then $\tilde{R}_j$ to avoid the possible overlaps in $\bigcup_{j=1}^M R_j$.</span> such that 
...
<span class="real-analysis-footnote">Note: Since $\chi_{R_j}(x)$ is defined on $R_j$, which is differ by a set of measure $\le 2\epsilon$.</span>
...
\]
<span class="real-analysis-footnote">Note: That is, $f(x) = \psi(x)$ for all $x$ at the outside of $E \Delta \bigcup_{j=1}^M R_j$.</span>
...
*Proof.*
We first assume that $f_k \to f$ on $E$. \\
For each $k, n \ge 1$, let 
\[
E_k^n = \left\{ x \in E : |f_j(x) - f(x)| < \frac{1}{n} \quad \forall j > k \right\}. 
\]
<span class="real-analysis-footnote">Note: *i.e.* the set $E_k^n$ is set of all $x \in E$ such that every $f_j$ with $j \ge k$ satisfies $|f_j(x) - f(x)| < \frac{1}{n}$. Hence, 
    
<ol>

<li markdown="1">If $k = 1$, then $x \in E_{1}^n$ must satisfy $|f_j(x) - f(x)| < \frac{1}{n}$ for all $j = 2, 3, 4, 5, \dots $
</li>
<li markdown="1">If $k = 2$, then $x \in E_{2}^n$ must satisfy $|f_j(x) - f(x)| < \frac{1}{n}$ for all $j = 3, 4, 5, \dots $
    
</li>
</ol>

    Therefore, as $k$ increases, the condition for $x$ to be in $E_k^n$ becomes weaker, and hence $E_k^n$ becomes larger. Thereby, for fixed $n$, the set $E_k^n$ is monotone increasing with respect to $k$.</span>
Note that for fixed $n$, $E_k^n \subset E_{k+1}^n \nearrow E$ as $k \to \infty$. 
Thus, by the result `cor:3.3`<span class="real-analysis-footnote">Note: Note that Corollary 3.3 says that if $E_{k+1}^n \nearrow E$, then $m(E) = \lim_{k \to \infty} m(E_k^n)$.</span>, 
\[
...
\begin{align*}
    ...
    &= \bigcup_{n \ge N} E \cap (E_{k_n}^n)^c \tag{$\because$ De Morgan's Law} \
...
\[
|f_j(x) - f(x)| < \frac{1}{n} < \delta \quad \forall \: j > k_n.
\]
...
\[
m(E \setminus F_\epsilon) \le \epsilon \quad \text{ and } \quad f|_{F_\epsilon} \text{ is continuous.}
\]
...
<span class="real-analysis-footnote">Note: Therefore, $E_n$ contains discontinuity points of $f_n$.</span> with 
\[
